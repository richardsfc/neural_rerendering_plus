...
def segment_images(images_path, xception_frozen_graph_path, save_dir,
    ...
    with tf.Session(graph=graph) as sess:  # TODO: Use tf.compat.v1.Session
        for i, img_path in enumerate(images_path):
            ...
            if len(img.shape) == 2 or img.shape[2] != 3:
                ...
                # remove the '_reference.png' suffix
                img_base_path = img_path[:-14]
            ...
            # convert to uint8 and squeeze to WxH.
            seg = np.squeeze(np.uint8(seg))
            parent_dir, filename = osp.split(img_path)
            basename, ext = osp.splitext(filename)
