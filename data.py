...
def _parser_rendered_dataset(
    ...
    height = tf.dtypes.cast(features['height'], tf.int32)
    width = tf.dtypes.cast(features['width'], tf.int32)

    # Parse the rendered image.
    rendered = tf.decode_raw(features['rendered'], tf.uint8)
    rendered = tf.dtypes.cast(rendered, tf.float32) * (2.0 / 255) - 1.0
    # Cast rendered to (height x width x 4)
    rendered = tf.reshape(rendered, [height, width, 4])
    if not use_alpha:
        # Only take first 3 layers, where originally have 4
    ...
    if use_depth:
        ...
        depth = tf.dtypes.cast(depth, tf.float32) * (2.0 / 255) - 1.0
        # Add another layer called "height" to input
    ...
    if use_semantics:
        ...
        seg_img = tf.dtypes.cast(seg_img, tf.float32) * (2.0 / 255) - 1
        # Add another layer called "seg" to input
    ...
    real = tf.dtypes.cast(real, tf.float32) * (2.0 / 255) - 1.0
    ...
    if crop_flag:
        ...
        if seeds is not None and len(seeds) <= 1:
            ...
            if not seeds:  # random crops
                ...
                seed = seeds[seed_idx]
            conditional_input = tf.image.random_crop(
